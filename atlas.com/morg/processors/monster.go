...
func (m *Monster) StartControl(monster *models.Monster, controllerId int) {
	...
	um := registries.GetMonsterRegistry().ControlMonster(monster.UniqueId(), controllerId)
	if um != nil {
		producers.NewMonsterControl(m.l, context.Background()).EmitControl(um.WorldId(), um.ChannelId(), um.MapId(), um.ControlCharacterId(), um.UniqueId())
	}
}

func (m *Monster) StopControl(monster *models.Monster) {
	um := registries.GetMonsterRegistry().ClearControl(monster.UniqueId())
	if um != nil {
		producers.NewMonsterControl(m.l, context.Background()).EmitStop(um.WorldId(), um.ChannelId(), um.MapId(), um.ControlCharacterId(), um.UniqueId())
	}
}
